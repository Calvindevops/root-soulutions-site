...
export function SectionDivider({ topColor, bottomColor, variant, flip = false }: SectionDividerProps) {
  if (variant === "leaf-wave") {
    return (
      <div className={`w-full overflow-hidden leading-[0] relative ${flip ? "rotate-180" : ""}`} style={{ backgroundColor: bottomColor }}>
        <svg
          viewBox="0 0 1440 80"
  ...
  // Root/Beet pattern with beetroot accents
  return (
    <div className={`w-full overflow-hidden leading-[0] relative ${flip ? "rotate-180" : ""}`} style={{ backgroundColor: bottomColor }}>
      <svg
        viewBox="0 0 1440 80"
      ...
      {/* Beetroot accents */}
      <div className="absolute top-1/2 -translate-y-1/2 left-[15%] opacity-40 hidden md:block">
        <Image src="/brand/beetroot-small.png" alt="" width={32} height={20} />
      </div>
      <div className="absolute top-1/2 -translate-y-1/2 left-[50%] -translate-x-1/2 opacity-50">
        <Image src="/brand/beetroot-small.png" alt="" width={40} height={25} />
      </div>
      <div className="absolute top-1/2 -translate-y-1/2 right-[15%] opacity-40 hidden md:block">
        <Image src="/brand/beetroot-small.png" alt="" width={32} height={20} />
      </div>
    </div>
  );
